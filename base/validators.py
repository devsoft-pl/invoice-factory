...
phone_number_validator = optional_regex_validator(
    r"^[0-9]{9,}$", _("Please enter a phone with a minimum 9 numbers")
)

max_day_validator = MaxValueValidator(31)

polish_nip_validator = optional_regex_validator(
    r"^\d{10}$",
    _("Polish NIP must consist of 10 digits."),
)

polish_zip_code_validator = optional_regex_validator(
    r"^\d{2}-\d{3}$",
    _("Polish ZIP code must be in the format XX-XXX."),
)

foreign_zip_code_validator = optional_regex_validator(
    r"^[A-Za-z0-9\-\s]{2,15}$",
